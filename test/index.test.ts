...
describe('rfc2606-domains', () => {
  describe('valid', () => {
    ...
    it('skips Html, Image, Link, Definition with ignoreNodes', async () => {
      expect(
        await lintMd('<a href="https://your-domain.com">link</a>', {
          ignoreNodes: ['Html'],
        }),
      ).toEqual([]);
      expect(
  ...
  describe('markdown support', () => {
    ...
    it('allows RFC 2606 domains in reference-style link definitions', async () => {
      expect(await lintMd('[mylink]: https://example.com/docs')).toEqual([]);
    });

    it('fixes placeholder domains in reference-style link definitions', async () => {
      expect(await fixMd('[mylink]: https://your-domain.com/docs')).toBe(
        '[mylink]: https://example.com/docs',
      );
    });
  });
  ...
  describe('HTML node support', () => {
    ...
    it('allows RFC 2606 domains in HTML', async () => {
      expect(await lintMd('<a href="https://example.com">docs</a>')).toEqual(
        [],
      );
    });

    it('fixes placeholder domains in HTML', async () => {
      expect(await fixMd('<a href="https://your-domain.com">click</a>')).toBe(
